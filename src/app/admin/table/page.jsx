...
export default function Table({ darkMode }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (modalMode === 'add') {
                try {
                    const response = await axios.post('/api/meja/add', formData,{
                        headers : {
                ...
                try {
                    const response = await axios.put(`http://localhost:5000/api/v1/meja/edit/${currentTable.id}`, formData, {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
                        withCredentials: true,
                    ...
                    if (err.response && err.response.status === 401) {
                        try {
                            ...
                            const response = await axios.put(`/api/meja/edit/${currentTable.id}`, formData, {
                                headers: {
                                    Authorization: `Bearer ${token}`,
                                },
                                withCredentials: true,
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await axios.delete(`/api/meja/delete/${currentTable.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                withCredentials: true,
            ...
            if(error.response?.data?.message){
                ...
                setError('Failed to delete Tabbel item');
            }
        }
    };
    ...
    const filteredTable = (Array.isArray(table)?table : []).filter(table => 
        table.table_number?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        String(table.capacity)?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const currentTables = filteredTable.slice(indexOfFirstTable, indexOfLastTable);
                    ...
                    <table className="min-w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-xs md:text-xl">
                        <thead>
                            <tr className="bg-gray-100 dark:bg-gray-700">
                                ...
                                <th className="py-3 px-4 text-left font-semibold border-b border-gray-200 dark:border-gray-600">Table Number</th>
                                <th className="py-3 px-4 text-left font-semibold border-b border-gray-200 dark:border-gray-600">Capacity</th>
                                <th className="py-3 px-4 text-left font-semibold border-b border-gray-200 dark:border-gray-600">Satus</th>
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan="5" className="py-4 text-center">Tidak ada data Meja</td>
                                </tr>
                            ) : (
                                currentTables.map((table, index) => (
                                    <tr key={table.id} className={`${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700'} hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-150`}>
                                        <td className="py-3 px-4 border-b border-gray-200 dark:border-gray-600">{indexOfFirstTable + index + 1}</td>
                                        <td className="py-3 px-4 border-b border-gray-200 dark:border-gray-600 ">{table.table_number}</td>
                                        <td className="py-3 px-4 border-b border-gray-200 dark:border-gray-600">{table.capacity}</td>
                                        <td className="py-3 px-4 border-b border-gray-200 dark:border-gray-600">
                                            <span className={`px-2 py-1 rounded-full text-xs ${table.status === 'occupied' ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'}`}>
                                                {table.status}
                                            </span>
                                        </td>
                                        <td className="py-3 px-4 border-b border-gray-200 dark:border-gray-600">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
                            ...
                            <div className="border-b border-gray-200 dark:border-gray-700 p-4">
                                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                                    {modalMode === 'add' && 'Tambah Meja'}
                                    {modalMode === 'edit' && 'Edit Meja'}
                                    {modalMode === 'delete' && 'Hapus Meja'}
                                </h3>
                                <button
                                ...
                                {modalMode === 'delete' ? (
                                    ...
                                    <div>
                                        <p className="text-gray-700 dark:text-gray-300 mb-4">
                                            Anda yakin ingin menghapus user <span className="font-semibold">{currentTable?.table_number}</span>?
                                        </p>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <select
                                                ...
                                                required
                                                // className="w-full p-2 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                                className={`w-full p-2 rounded-md border border-gray-300 dark:border-gray-600 ${formData.status == 'available' ? 'bg-green-600 text-gray-900 dark:bg-green-700 dark:text-white'  : 'bg-red-600 text-gray-900 dark:bg-red-700 dark:text-white' }`}
                                                >
                                                <option value="" disabled hidden>Pilih Satus</option>
